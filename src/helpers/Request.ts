...
const instance = axios.create({
  baseURL: 'http://172.31.77.220:8000/api/',
  headers: {
...
const requestGet = async (path: string) => {
  const getToken = await AsyncStorage.getItem('userToken')
  const token = getToken || TOKEN_BONDAN
  ...
  return instance.get(path, { headers }).then(v => v.data)
}

export const getDetail = () => requestGet('details')
